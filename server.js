...
function gateKeeper(req, res, next) {
  ...
  console.log("hello gateKeeper");
  
  let getUserPass = req.get('x-username-and-password');
  let parseCreds = queryString.parse(getUserPass);
  
  console.log(getUserPass);
  console.log(parseCreds);
  
  let findUsername = function(user) {
    return user.userName === parseCreds.user;
  }
  
  let findPassword = function(user) {
    return user.password === parseCreds.pass;
  }
  
  let checkUsername = USERS.find(findUsername);
  // let checkPassword = USERS.find(findPassword);
  ...
  if (getUserPass) {
      if (checkUsername) {
            ...
            req.user = USERS[checkUsername.id - 1];
...
// app.use(gateKeeper);  --- alternatively, I chose to add it directly to the route below
